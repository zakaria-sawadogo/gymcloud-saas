...
async function main() {
  ...
  await prisma.saasInvoice.create({
    data: {
      ...
      invoiceNumber: `GC-SAAS-DEMO-${randomUUID().slice(0, 8).toUpperCase()}`,
  ...
  await prisma.gestionnaireProfile.create({
    data: { id: randomUUID(), userId: gestionnaireUser.id, salleId: salle.id },
  });
  ...
  const [mensuel, trimestriel, annuel] = await Promise.all([
    prisma.abonnementCatalogue.create({
      data: { id: randomUUID(), salleId: salle.id, name: 'Mensuel', durationDays: 30, price: 15000, currency: 'XOF' },
    }),
    prisma.abonnementCatalogue.create({
      data: { id: randomUUID(), salleId: salle.id, name: 'Trimestriel', durationDays: 90, price: 40000, currency: 'XOF' },
    }),
    prisma.abonnementCatalogue.create({
      data: { id: randomUUID(), salleId: salle.id, name: 'Annuel', durationDays: 365, price: 140000, currency: 'XOF' },
    }),
  ]);
  ...
  const adherentsPlan: Array<{
    firstName: string;
    lastName: string;
    phone: string;
    formule: typeof mensuel;
    daysAgoStart: number; // début de l'abonnement, il y a N jours
    payerStatus: 'VALIDE' | 'EN_ATTENTE';
  }> = [
  ...
  for (const a of adherentsPlan) {
    ...
    await prisma.receipt.create({
      data: {
        ...
        number: `GC-REC-DEMO-${randomUUID().slice(0, 8).toUpperCase()}`,
      },
    });
  }
